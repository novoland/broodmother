...
public class CrawlJob {
	...
	private AbstractRefreshableConfigApplicationContext container;
	...
	private void prepareAppContext(File jobDir) {
        ...
        prepareAppContext(jobClassLoader,jobConfigFile);
	}

    private void prepareAppContext(URLClassLoader jobClassLoader, String jobConfigFile) {
    ...
    private void prepareAppContext(String jobXmlFileName){
        container = new ClassPathXmlApplicationContext();
        container.setConfigLocation(jobXmlFileName);
        container.refresh();
    }
     ...
     *
     * @param name
     * @param jobXmlFileName
     */
    public CrawlJob(String name, String jobXmlFileName){
        this.name = name;
        try {
            // initiate the spring container
            prepareAppContext(jobXmlFileName);
            crawlController = container.getBean(CrawlController.class);
            crawlController.setCrawlJob(this);
        } catch (Throwable e) {
            throw new CrawlJobException(e);
