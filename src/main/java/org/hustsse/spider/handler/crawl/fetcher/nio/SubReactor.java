...
public class SubReactor implements Runnable {
	...
	Logger logger = LoggerFactory.getLogger(SubReactor.class);
	...
	public SubReactor(Executor reactorExecutor, int index) {
    ...
    class RegisterTask implements Runnable {
		...
		public void run() {
			try {
				...
				Boolean requestSendFinished = (Boolean) uri.getAttr(_REQUEST_SEND_FINISHED);
	...
	private void processWritableKey(SelectionKey key) {
		...
		ByteBuffer buffer = (ByteBuffer) url.getAttr(_REQUEST_BUFFER);
		try {
			...
			for (int i = WRITE_SPIN_COUNT; i > 0; i--) {
				...
				// write success
				if (writtenBytes != 0) {
					url.setAttr(_LAST_SEND_REQUEST_MILLIS, System.currentTimeMillis());
					url.setAttr(_REQUEST_ALREADY_SEND_SIZE, (Integer) url.getAttr(_REQUEST_ALREADY_SEND_SIZE) + writtenBytes);
					url.setAttr(_REQUEST_SEND_TIMES, (Integer) url.getAttr(_REQUEST_SEND_TIMES) + 1);
					break;
				}
			}
			boolean reqSendFinished = !buffer.hasRemaining();
			url.setAttr(_REQUEST_SEND_FINISHED, reqSendFinished);
			url.setAttr(_REQUEST_SEND_FINISHED_MILLIS, reqSendFinished);
			if (reqSendFinished) {
	...
	private void processReadableKey(SelectionKey key) {
		...
		try {
			...
			Object lastSendTime = uri.getAttr(_LAST_SEND_REQUEST_MILLIS);
			Long conTime = (Long) uri.getAttr(_CONNECT_SUCCESS_MILLIS);
			Integer sendReqTimes = (Integer) uri.getAttr(_REQUEST_SEND_TIMES);
			Integer sendBytes = (Integer) uri.getAttr(_REQUEST_ALREADY_SEND_SIZE);
			Integer requestSize = (Integer) uri.getAttr(_REQUEST_SIZE);
